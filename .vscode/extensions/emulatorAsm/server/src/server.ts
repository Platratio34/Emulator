...
connection.onDidChangeConfiguration(change => {
    if (hasConfigurationCapability) {
        ...
        globalSettings = (
            (change.settings.emulatorAsmServer || defaultSettings)
        );
    }
...
async function validateTextDocument(textDocument: TextDocument): Promise<Diagnostic[]> {
    ...
    const addDiag = (severity: DiagnosticSeverity, start: number, end: number, message: string) => {
    ...
    const addError = (start: number, end: number, message: string) => {
        addDiag(DiagnosticSeverity.Error, start, end, message)
        problems++;
    }
    const addWarning = (start: number, end: number, message: string) => {
        addDiag(DiagnosticSeverity.Warning, start, end, message)
        problems++;
    }
    const addInfo = (start: number, end: number, message: string) => {
        addDiag(DiagnosticSeverity.Information, start, end, message)
    }
    const addHint = (start: number, end: number, message: string) => {
        addDiag(DiagnosticSeverity.Hint, start, end, message)
    }
...
type AsmLine = { name: string, desc: string, usage: string, sub?: { [id: string]: AsmLine } }
const compilerLines: { [id: string]: AsmLine } = {
